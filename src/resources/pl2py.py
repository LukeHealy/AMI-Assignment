...
import AI.Pathfinding.SMAstar.PriorityQueue
import AI.Pathfinding.SMAstar.Path
import Scalar.Util
import Carp
...
def add_start_state::
    ...
    state_eval_func = self._state_eval_func
    state_goal_p_func = self._state_goal_p_func
    state_num_successors_func = self._state_num_successors_func,
    state_successors_iterator = self._state_successors_iterator,
    state_get_data_func = self._state_get_data_func
    
    # make sure required functions have been defined
    if( not defined(state_eval_func)):
    croak "SMAstar:  evaluation function is not defined\n"
    
    if( not defined(state_goal_p_func)):
    croak "SMAstar:  goal function is not defined\n"
    
    if( not defined(state_num_successors_func)):
    croak "SMAstar:  num successors function is not defined\n"
    
   if( not defined(state_successors_iterator)):
    croak "SMAstar:  successor iterator is not defined\n"
    

    # create a path object from this state
    state_obj = AI.Pathfinding.SMAstar.Path.new(
    _state           => state,
    ...
    fcost = AI.Pathfinding.SMAstar.Path.fcost(state_obj)
    ...
    num_successors = state_obj.get_num_successors()
    ...
    classname = ref(state)
    test_successor_iterator = state_obj._successors_iterator.(state)
    test_successor = test_successor_iterator.(state)
    succ_classname = ref(test_successor)
...
def sma_star_tree_search::
    ...
    iteration = 0
    num_states_in_queue = priority_queue.size()
    max_extra_states_in_queue = max_states_in_queue
    max_states_in_queue = num_states_in_queue + max_extra_states_in_queue;    
    max_depth = (max_states_in_queue - num_states_in_queue)

     # the best candidate for expansion


    
    if(priority_queue.is_empty() ||  not priority_queue):
    return
    ...
    while( not priority_queue.is_empty()):
        ...
        if(best.goal_p()) {          
        # goal achieved not  iteration: iteration, number of 
        ...
        successors_iterator = best.successors_func();        
        succ = successors_iterator.()
            
        if(succ):
            # if succ is at max depth and is not a goal node, set succ.fcost to infinity 
            if(succ.depth() >= max_depth &&  not succ.goal_p() ){                       
            ...
            descendant_index = succ._descendant_index
        ...
        if(best.is_completed()):
            ...
            fval_before_backup = best._f_cost
            ...
            while(antecedent):
                path_str = str_function.(antecedent);    
        ...
        if(num_states_in_queue > max_states_in_queue):
            shcl_obj = priority_queue.shallowest_highest_cost_leaf(best, succ, str_function); 
...
1
__END__
# Below is stub documentation for your module+ You'd better edit it not 
...
head2 EXAMPLE
 ...
 #
 frontierGoalPath = smastar.start_search(
...
head2 start_search()
  ...
  frontierGoalObj = smastar.start_search(
    \&log_function,       # returns a string used for logging progress
    \&str_function,       # returns a string used to *uniquely* identify a node 
    max_states_in_queue, # indicate the maximum states allowed in memory
    MAX_COST,            # indicate the maximum cost allowed in search
